...
async function permissoesDe(usuario) {
  if (!usuario) return [];
  if (usuario.papel === 'superadmin') return [...CHAVES];
  const gravadas = (await db.prepare('SELECT permissao FROM usuario_permissoes WHERE usuario_id = ?')
    .all(usuario.id)).map(r => r.permissao);
  return normalizarPermissoes(gravadas);
}

async function temPermissao(usuario, chave) {
  return (await permissoesDe(usuario)).includes(chave);
}
