...
router.get('/minhas', async (req, res) => {
  ...
  const agendamentos = await db.prepare('SELECT * FROM agendamentos WHERE usuario_id = ? ORDER BY data_servico DESC').all(req.session.usuario.id);
  res.json(agendamentos);
});

// Gestao (admin/superadmin)
router.get('/', exigirPapel('admin', 'superadmin'), async (req, res) => {
...
router.put('/:id/deferir', exigirPapel('admin', 'superadmin'), async (req, res) => {
  const agendamento = await db.prepare('SELECT * FROM agendamentos WHERE id = ?').get(req.params.id);
  if (!agendamento) return res.status(404).json({ erro: 'Agendamento não encontrado.' });
  await db.prepare(`UPDATE agendamentos SET status = 'aprovado', motivo_recusa = NULL, atualizado_em = datetime('now') WHERE id = ?`).run(agendamento.id);
  res.json({ ok: true });
});

router.put('/:id/indeferir', exigirPapel('admin', 'superadmin'), async (req, res) => {
  const { motivo } = req.body || {};
  const agendamento = await db.prepare('SELECT * FROM agendamentos WHERE id = ?').get(req.params.id);
  if (!agendamento) return res.status(404).json({ erro: 'Agendamento não encontrado.' });
  await db.prepare(`UPDATE agendamentos SET status = 'recusado', motivo_recusa = ?, atualizado_em = datetime('now') WHERE id = ?`).run(motivo || null, agendamento.id);
  res.json({ ok: true });
});

router.put('/:id/concluir', exigirPapel('admin', 'superadmin'), async (req, res) => {
  await db.prepare(`UPDATE agendamentos SET status = 'concluido', atualizado_em = datetime('now') WHERE id = ?`).run(req.params.id);
  res.json({ ok: true });
});
